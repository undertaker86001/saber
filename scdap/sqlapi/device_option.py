...
class DeviceOptionKey(CommonFlag):
    tag = 'tag'
    devices = 'devices'
    worker = 'worker'
...
flag = DeviceOptionKey


class DeviceOptionItem(Base, DapBaseItem):
...
_api = TableApi(DeviceOptionItem)
...
def get_option(tag: Union[str, List[str], None] = None,
    ...
    return _api.query_with_unique(DeviceOptionItem.tag, tag, enabled, session=session)
...
def get_tags(enabled: Optional[bool] = None, session=None) -> List[int]:
    ...
    return _api.get_exist_keys_with_unique(DeviceOptionItem.tag, enabled, session=session)
...
def tag_exist(tag: Union[str, List[str]], enabled: Union[bool, int] = None, last_one: bool = False, session=None) \
    ...
    check_type(enabled, 'enabled', (bool, int), True)
    return _api.check_exist_with_unique(DeviceOptionItem.tag, tag, enabled, last_one, session=session)
...
def update_option(tag: str, worker: str = None,
    ...
    _api.update_with_unique(DeviceOptionItem.tag, tag, kv, session=session)
...
def delete_option(tag: Union[List[str], str], session=None):
    ...
    _api.delete_with_unique(DeviceOptionItem.tag, tag, session=session)
